...
EigensystemExact[m_]:=Module[{vals,vecs,i,blocksizes,ord,out,block,block2,startrow},{vals,vecs}=Eigensystem[m];ord=Ordering[vals];vals=vals[[ord]];vecs=vecs[[ord]];Off[Tally::smtst];blocksizes=Transpose[Tally[vals,Chop[Simplify[#1 - #2]]==0&]][[2]];On[Tally::smtst];startrow = 1;out={};For[i=1,i<=Dimensions[blocksizes][[1]],i++,block=Take[vecs,{startrow,startrow-1+blocksizes[[i]]}];block2=Simplify[Orthogonalize[block]];block2=Simplify[Map[Normalize[#]&,block2]];startrow = startrow + blocksizes[[i]];out=Join[out,block2]];{vals,out}]

SimultaneouslyDiagonalize[A_, B_, precision_: None]:=Module[{vals, vecs, unitary, blocksizes, startrow, out, i, Bblock, vals2, vec2, unitary2, A1, B1, mA, nA, mB, nB, valA, valB, vec,ord}, {mA, nA} = Dimensions[A]; {mB, nB} = Dimensions[B]; If[A.B - B.A != ConstantArray[0, {mA, nA}], A1 = Chop[A]; B1 = Chop[B], A1 = A; B1 = B]; {vals, vecs} = EigensystemExact[A1]; ord=Ordering[vals];vals=vals[[ord]];vecs=vecs[[ord]]; unitary = Map[Normalize[#] &, vecs]; blocksizes = Transpose[Tally[vals,If[NumericQ[precision], Chop[#1 - #2, precision] == 0 &,Chop[#1 - #2] == 0 &]]][[2]]; startrow = 1; out = {};For[i = 1, i <= Dimensions[blocksizes][[1]],i++,Bblock=Chop[Take[unitary.B1.CT[unitary],{startrow,startrow-1+blocksizes[[i]]},{startrow,startrow - 1 + blocksizes[[i]]}]];startrow = startrow + blocksizes[[i]];If[Bblock == DiagonalMatrix[Diagonal[Bblock]],unitary2 = IdentityMatrix[Dimensions[Bblock][[1]]], {vals2, vec2}=EigensystemExact[Bblock]; unitary2 = Map[Normalize[#] &, vec2]];If[i == 1, out = unitary2, out = DirectSum[out, unitary2]]]; out = out.unitary; {valA, valB, vec} = {Diagonal[out.A1.CT[out]], Diagonal[out.B1.CT[out]], out};(* The next If is not needed, but can be useful to flag problems *) If[Chop[N[CT[vec].DiagonalMatrix[valA].vec - A1]] != ConstantArray[0, {mA, nA}] || Chop[N[CT[vec].DiagonalMatrix[valB].vec - B1]] != ConstantArray[0, {mA, nA}],Print["Error in SimultaneouslyDiagonalize with inputs ",A,", ",B, ", ", precision]];CT[vec]]
